...
### `daily_quote` (홈 헤드라인) — **밈 금지** (2026-07-27 개편)

홈 첫 화면 제목이라 개그 욕심이 제일 크게 튄다. 유행어 톤을 전면 철회했다.

- **밈·유행어를 쓰지 않는다.** 담백한 속담·관용구 결 + 살짝 미는 넛지.
- 짧은 완결형 문장. 21px 헤드라인 기준 **2줄이 한계**, 3줄은 넘기지 않는다.
- 넛지는 반 발짝만 — 훈수·명령조로 넘어가지 않게 한다.
- 리서치 에이전트는 이 surface 에 밈 문구를 제안하지 않는다.

### `fortune_overall` / `fortune_advice` (행운지수) — 유행어 허용
