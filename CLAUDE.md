...
- **커밋한 즉시 현재 브랜치로 푸시한다.** 원격에 올라가야 배포 서버가 받아서
  거기서 바로 확인할 수 있다. 기능 몇 개를 모아 뒀다가 한꺼번에 푸시하지 않는다 —
  기능 하나 끝날 때마다 커밋 1개 + 푸시 1번.
- 푸시가 실패하면 거기서 멈추지 말고 원인을 알린다. 커밋만 해 두고 "나중에 푸시하세요"로
  끝내지 않는다 (예: 워크플로 파일이 섞여 `workflow` 스코프가 없어 거부되는 경우 —
  `gh auth refresh -s workflow` 가 필요하다는 걸 바로 말해 준다).
